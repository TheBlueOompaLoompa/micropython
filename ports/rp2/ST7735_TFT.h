...
static void st7735_set_region(uint16_t x_start, uint16_t x_end, uint16_t y_start, uint16_t y_end) {
  tft_cs_low();

  spi_command(ST7735_CASET);
  spi_data((x_start >> 8) & 0xFF, x_start & 0xFF, (x_end >> 8) & 0xFF, x_end & 0xFF);

  spi_command(ST7735_RASET);
  spi_data((y_start >> 8) & 0xFF, y_start & 0xFF, (y_end >> 8) & 0xFF, y_end & 0xFF);
...
static void st7735_fill_start() {
  st7735_set_region(0x0000, 0x007F, 0x0000, 0x009F);
}

#define RBG(c) (c & ST7735_RED) | ((c & ST7735_BLUE) << 6) | ((c & ST7735_GREEN) >> 6)
